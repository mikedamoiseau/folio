...
/**
 * Non-blocking, dismissible "preparing pages" bar for the progressive
 * comic-open feature (F-4-1). Floats over the page viewer while the
 * backend extracts the remaining pages in the background — it never gates
 * reading or navigation (which the backend serves on demand). Visibility
 * is owned by the caller via `isComicExtractProgressVisible`; this
 * component only renders the chrome.
 */
export default function ComicExtractProgress({ loaded, total, onDismiss }: ComicExtractProgressProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-ink-muted tabular-nums whitespace-nowrap">
        {t("reader.preparingPagesProgress", { loaded, total })}
      </span>
