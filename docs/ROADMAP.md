...
3. ~~**PDF disk cache — P3.**~~ — **shipped 2026-05-17.** First-open warm of 10 pages + lazy fill, all under the existing `page-cache/{hash}/` namespace. Cached at canonical 2400 px and downscaled per viewport request. Shares the comic cache budget, LRU, and 7-day eviction; lazy writes coalesce into a background eviction every 25 pages. Eviction reads filesystem-truth (`book_disk_size_bytes`) so concurrent manifest writes can't drift the budget. Linked / no-hash PDFs (or storage errors) gracefully bypass the cache and render at the viewport width. ([CHANGELOG](../CHANGELOG.md#unreleased))
